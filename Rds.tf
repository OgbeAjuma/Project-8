resource "aws_db_instance" "my-proj8sql-db" {
...
resource "aws_db_subnet_group" "my-p8rds-db-subnet" {
  ...
  subnet_ids = [aws_subnet.privsub-1.id, aws_subnet.privsub-2.id]
...
resource "aws_security_group" "rds-SG-proj8" {
  name        = "rds-SG-proj8"
...
resource "aws_security_group_rule" "my-p8rds-sg-rule" {
  ...
  security_group_id = aws_security_group.rds-SG-proj8.id
}
